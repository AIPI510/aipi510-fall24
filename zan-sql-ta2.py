...
try:
    ...
    with sqlite3.connect('tips.db') as conn:
        ...
        query_add_2="""
        SELECT sex, day, ROUND(SUM(total_bill),2), SUM(tip)
        FROM tips
        GROUP BY sex, day
        """
        ...
        for row in result_1:
            ...
            print(f"Day: {day}, Average Tip Percentage: {avg_tip_percentage}%")
        ...
        for row in result_5:
            ...
            print(f"Day: {day}, Time: {time}, Total Bill: ${total_bill}, Total Tip: ${total_tip}, Average Tip Percentage: {avg_tip_percentage}%")

        print('---Question6---')
        for row in result_6:
            day, time, smoke, avg_tip_percentage= row
            print(f"Day: {day}, Time: {time}, Smoke Status: {smoke}, Avg tip percentage:{avg_tip_percentage}")  

        
        print('---Question7---')
        for row in result_7:
            sex, total_bill, tip, tip_percentage= row
            print(f"Sex: {sex}, Total Bill: {total_bill}, Tip: {tip}, Tip Percentage:{tip_percentage}")  

        print('---Question8---')
        for row in result_8:
            day, time, total_bill, tip, tip_percentage, tip_type = row
            print(f"Day: {day}, Time: {time}, Total Bill: ${total_bill}, Tip: {tip}, Tip Percentage:{avg_tip_percentage}, Tip Type:{tip_type}") 

        print('---Question9---')
        for row in result_9:
            total_bill, tip, avg_tip_percentage= row
            print(f"Total bill: {total_bill}, Tip: {tip}, Avg tip percentage:{avg_tip_percentage}")

        print('---Question10---')
        for row in result_10:
            day, time, smoker, tip_percentage = row
            print(f"Day: {day}, Time: {time}, Smoker: {smoker}, Tip percentage:{tip_percentage}")
        ...
        for row in result_add_1:
            ...
            print(f"Day: {day}, Sex: {sex}, Avg tip percentage: {avg_tip_percentage}%") 
